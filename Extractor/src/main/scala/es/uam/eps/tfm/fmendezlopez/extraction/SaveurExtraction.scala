...
/**
  ...
  */
object SaveurExtraction extends Logging{
  ...
  def main(args: Array[String]): Unit = {
    ...
    connectionProperties.put("timeout", properties.getProperty("stage7.scraping.delay.timeout"))
    connectionProperties.put("max-body-size", properties.getProperty("stage7.scraping.maxBodySize"))
    connectionProperties.put("follow-redirects", properties.getProperty("stage7.scraping.followRedirects"))
    connectionProperties.put("host", properties.getProperty("saveur.host"))
    connectionProperties.put("referrer", "")
    connectionProperties.put("attempts", properties.getProperty("stage7.scraping.maxAttempts"))
    connectionProperties.put("delay-detection", properties.getProperty("stage7.scraping.time.detection"))
    connectionProperties.put("base-host", properties.getProperty("saveur.url.base"))
    connectionProperties.put("base-recipe", properties.getProperty("saveur.url.base"))
    ...
    val outCSVDelimiter = properties.getString("stage7.output.csv.delimiter")
    val csvRecipesName : String = properties.getString("stage7.output.csv.recipes.name")
    val csvStepsName : String = properties.getString("stage7.output.csv.steps.name")
    val csvIngredientsName : String = properties.getString("stage7.output.csv.ingredients.name")
    ...
    csvRecipes.writeRow(Utils.headerToSeq(properties.getString("stage7.output.csv.recipes.header"), outCSVDelimiter.charAt(0)))
    csvSteps.writeRow(Utils.headerToSeq(properties.getString("stage7.output.csv.steps.header"), outCSVDelimiter.charAt(0)))
    csvIngredients.writeRow(Utils.headerToSeq(properties.getString("stage7.output.csv.ingredients.header"), outCSVDelimiter.charAt(0)))
  ...
  def extractURLs(diffMap : Map[String, Seq[File]]) : Map[String, Seq[String]] = {
    ...
    val maxrecipes = properties.getInt("stage7.scraping.recipesperdifficulty")
    var id = 0
    ...
    diffMap.foreach({case (difficulty, list) =>
      ...
      do{
        val file = it.next()
        val list = Scraper.scrapeSaveurURLs(file, base_recipe_url, nrecipes)
        urls ++= list
  ...
  def extractRecipes(outCSVDelimiter : String, urls : Map[String, Seq[String]]) : Unit = {
    ...
    urls.foreach({case(difficulty, urlList) =>
      ...
      val recipeList : Seq[CookipediaRecipe] = urlList.flatMap(url => {
        ...
        val potRecipe = Scraper.scrapeSaveurRecipe(page, '|')
