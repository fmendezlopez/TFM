...
object Scraper extends Logging{
  ...
  def scrapeAuthorList(html: String): Seq[Author] = {

    def scrapeAuthor(article: Element): Option[Author] = {
      val divs = article >> elementList("div.fixed-recipe-card__profile")
      if(divs.length != 1){
        logger.error("Could not scrape author list")
        None
      }
      else{
        val div = divs.head
        val as = div >> elementList("a.ng-isolate-scope")
        if(as.length != 1)
          None
        else{
          val url = as.head.attr("href")
          val id = Utils.detectIDinURL(url, "/")
    ...
    val sections: List[Element] = body >> elementList("section#fixedGridSection")
    ...
    val articles = section >> elementList("article.fixed-recipe-card")
    articles.flatMap(scrapeAuthor)
  }
